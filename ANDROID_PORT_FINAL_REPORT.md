...
## Correções de fullscreen, controles touch e gamepad físico

O log `rodando15_08-20-39-39_428.log` confirmou que a renderização OpenGL, o áudio AAudio e o boot do disco estão estáveis. Ele também mostrou a origem dos três problemas: SDL solicitava uma janela redimensionável 960×720 sem fullscreen; o runtime tinha somente lógica de leitura das regiões touch, sem desenhar qualquer virtual gamepad; e o release Android inicializava P1 como `keyboard`, enquanto apenas builds de debug usavam `auto` para abrir um SDL GameController.

A revisão `e5a7642d5c04422d907724831acbfd910f915c14` adiciona fullscreen imersivo Java com ocultação das barras de sistema, força `SDL_WINDOW_FULLSCREEN_DESKTOP` no Android, cria `TouchOverlayView` visual e transparente aos eventos SDL, muda o padrão P1 Android para `auto`, e solicita `BLUETOOTH_CONNECT`/`BLUETOOTH_SCAN` em Android 12+ para gamepads Bluetooth. O runtime registra no Logcat a enumeração SDL e o controle aberto no P1. A execução [31915625317](https://github.com/deivid22srk/TwistedMetal4Recomp-Android/actions/runs/31915625317) terminou com sucesso.

A nova release está em [android-build-e5a7642d5c04](https://github.com/deivid22srk/TwistedMetal4Recomp-Android/releases/tag/android-build-e5a7642d5c04), com download direto em [app-release.apk](https://github.com/deivid22srk/TwistedMetal4Recomp-Android/releases/download/android-build-e5a7642d5c04/app-release.apk). O APK contém `classes.dex`, `libmain.so`, `libSDL3.so` em `lib/arm64-v8a/` e o OpenBIOS; `apksigner` confirmou V1/V2 válidos. SHA-256: `0098194d4e892d3902ef9fb7aff1ba1d1f2a02cf6630eda58b78bb46236f6bd3`.

## Layout touch aprimorado e alternância por gamepad

A imagem fornecida mostrou que o primeiro overlay ocupava áreas úteis do HUD e não oferecia L2/R2. A revisão `d2657943c5649c8777c4c1998bd21e16c95884b8` move os controles para as barras laterais pretas do formato 20:9: D-pad à esquerda, botões de face à direita, L2/L1 empilhados no topo esquerdo, R2/R1 no topo direito e Select/Start no topo central. A geometria visual e as regiões nativas de toque agora são idênticas, inclusive para L2/R2.

A `GameActivity` consulta `InputDevice` a cada 500 ms. O overlay fica oculto quando qualquer dispositivo `SOURCE_GAMEPAD` ou `SOURCE_JOYSTICK` está conectado e reaparece quando todos são desconectados. O controle touch continua sendo entregue ao SDL quando visível; a camada visual não intercepta os eventos.

A execução [31916376114](https://github.com/deivid22srk/TwistedMetal4Recomp-Android/actions/runs/31916376114) terminou com sucesso. O APK está na release [android-build-d2657943c564](https://github.com/deivid22srk/TwistedMetal4Recomp-Android/releases/tag/android-build-d2657943c564), com download direto em [app-release.apk](https://github.com/deivid22srk/TwistedMetal4Recomp-Android/releases/download/android-build-d2657943c564/app-release.apk). A assinatura V1/V2 foi verificada; o pacote contém `classes.dex`, `libmain.so`, `libSDL3.so` AArch64 e o OpenBIOS. SHA-256: `62c7d355a823c179141e1542b5605eb92bc8f12ebc1c419ab7a43e89e76cc576`.
