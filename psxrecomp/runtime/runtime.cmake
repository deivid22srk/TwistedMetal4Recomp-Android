...
if(_psx_sdl_backend STREQUAL "SDL3")
    ...
    if(NOT TARGET SDL3::SDL3 AND PSX_SDL3_FETCH)
        ...
        if(ANDROID)
            # SDLActivity loads libSDL3.so before libmain.so. Keep SDL as a
            # separate shared object on Android; desktop releases remain
            # self-contained with the historical static SDL target.
...
if(ANDROID)
    set(PSXRECOMP_FIBER_SOURCE
        "${CMAKE_CURRENT_LIST_DIR}/../../android/psx_fiber_android.c")
else()
    set(PSXRECOMP_FIBER_SOURCE
...
function(psxrecomp_add_runtime_target target)
    ...
    # SDL's Android Activity loads the application entry point from
    # libmain.so. Keep the normal executable on desktop platforms, but build
    # the exact same native sources as a shared library on Android. psx_sdl.h
    # includes SDL_main.h, so the exported entry symbol remains SDL_main.
    if(ANDROID)
